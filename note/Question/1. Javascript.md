...
- [x] What are the implications (ý nghĩa) of using closures in JavaScript?
...
- [x] What are common JavaScript security issues (e.g., XSS, CSRF), and how do you prevent them
  - `XSS (Cross-Site Scripting)`: Malicious(độc hại) scripts injected into web pages that can execute in a user's browser. Prevent with:
    - Input sanitization: Escape user inputs and avoid executing raw HTML.
    - Content Security Policy (CSP): Restrict(Hạn chế) where scripts can be loaded from.
